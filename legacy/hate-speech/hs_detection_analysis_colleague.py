...
def load_all_prompts_and_questions():
    ...
    for lang in LANGUAGES:
        ...
        if lang == 'slovene':
            q_path = f'./data/hate_speech/{lang}.csv'
        else:
        ...
        if os.path.exists(p_path) and os.path.exists(q_path):
            ...
            if lang == 'slovene':
                df = pd.read_csv(q_path, encoding='utf8', sep=',')
                for idx, row in df.iterrows():
                    if row['political_label'] == 'Desna':
                        target = 'Leva'
                    elif row['political_label'] == 'Leva':
                        target = 'Desna'
                    else:
                        target = 'Neznano'
                    tweet = {
                    ...
                    all_questions[lang + '_' + target].append(tweet)
            else:
                with open(q_path, 'r', encoding='utf-8') as f:
                    for i, line in enumerate(f, start=1):
                        ...
                        tweet = {
                            ...
                            "target": target,
                            # Combine dataset and grouping with a semicolon
